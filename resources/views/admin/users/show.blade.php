...
@section('title', 'Customer Profile - ' . $user->full_name)
...
@section('content')
    ...
    <div class="mx-auto max-w-5xl flex flex-col gap-8 animate-fade-up">
        ...
        <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-luxury-secondary/70">Registered on {{ $user->created_at->format('M d, Y') }}</span>
        </div>
        ...
        <div class="relative overflow-hidden rounded-3xl border border-white/10 bg-[#111113] p-6 sm:p-8 shadow-2xl shadow-black/40">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <div class="flex items-center gap-3">
                            <span class="font-display text-[10px] font-bold uppercase tracking-[0.28em] text-luxury-gold">Customer Profile</span>
                ...
                <form method="POST" action="{{ route('admin.users.toggle-status', $user) }}">
                    @csrf
                    <button type="submit" onclick="return confirm('Are you sure you want to {{ $user->is_active ? 'deactivate' : 'activate' }} this account?')" 
            ...
            <div class="lg:col-span-5 flex flex-col gap-6">
                <div class="rounded-3xl border border-white/10 bg-[#111113] p-6 sm:p-7 shadow-xl">
                    <div class="flex items-center gap-3 border-b border-white/10 pb-4 mb-6">
                        ...
                        <div>
                            <h2 class="font-display text-base font-bold text-white">Edit Profile</h2>
                            <p class="text-xs text-luxury-secondary/70">Update customer details</p>
                        </div>
                    </div>
                ...
                <div class="rounded-3xl border border-white/10 bg-[#111113] p-6 sm:p-7 shadow-xl">
                    <div class="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="font-display text-base font-bold text-white">Booked Appointments</h2>
                                <p class="text-xs text-luxury-secondary/70">Client reservation history</p>
                            </div>
                        </div>
                        <span class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-bold text-luxury-gold">
                            {{ $user->appointments->count() }} total
                        </span>
                    </div>

                    @if($user->appointments->isEmpty())
                        <p class="py-6 text-center text-xs text-luxury-secondary/70 italic">No appointments booked by this customer yet.</p>
                    @else
                        <div class="space-y-3">
                            @foreach($user->appointments->take(5) as $appointment)
                                <a href="{{ route('admin.appointments.show', $appointment) }}" class="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-xs transition hover:border-luxury-gold/40 hover:bg-white/[0.04]">
                                    <div class="flex flex-col gap-0.5">
                                        ...
                                        <span class="text-[11px] text-luxury-secondary">{{ $appointment->appointment_at->format('M d, Y \a\t H:i') }}</span>
                                    </div>
                                    <div class="flex items-center gap-3">
                ...
                <div class="rounded-3xl border border-white/10 bg-[#111113] p-6 sm:p-7 shadow-xl">
                    <div class="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="font-display text-base font-bold text-white">Product Orders</h2>
                                <p class="text-xs text-luxury-secondary/70">Shop purchase history</p>
                            </div>
                        </div>
                        <span class="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-bold text-luxury-gold">
                    ...
                    @if($user->orders->isEmpty())
                        <p class="py-6 text-center text-xs text-luxury-secondary/70 italic">No shop orders placed by this customer yet.</p>
                    @else
                        <div class="space-y-3">
