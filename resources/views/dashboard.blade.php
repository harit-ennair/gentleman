                ...
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <div class="flex items-center gap-3">
                            <span class="font-display text-[10px] font-bold uppercase tracking-[0.28em] text-luxury-gold">Client Dashboard</span>
                        ...
                        <p class="text-xs text-luxury-secondary max-w-lg">
                            Manage your upcoming grooming sessions, view product orders, and customize your profile.
                        </p>
                    </div>
                </div>
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <a href="{{ route('appointments.index') }}" class="group rounded-2xl border border-white/10 bg-[#111113] p-5 shadow-xl transition-all duration-300 hover:border-luxury-gold/40 hover:bg-white/[0.02]">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-luxury-secondary group-hover:text-luxury-gold transition-colors">Upcoming Bookings</span>
            ...
            <a href="{{ route('orders.index') }}" class="group rounded-2xl border border-white/10 bg-[#111113] p-5 shadow-xl transition-all duration-300 hover:border-luxury-gold/40 hover:bg-white/[0.02]">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-luxury-secondary group-hover:text-luxury-gold transition-colors">Product Orders</span>
                    <div class="grid size-8 place-items-center rounded-xl bg-luxury-gold/10 text-luxury-gold">
                        <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('profile.edit') }}" class="group rounded-2xl border border-white/10 bg-[#111113] p-5 shadow-xl transition-all duration-300 hover:border-luxury-gold/40 hover:bg-white/[0.02]">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-luxury-secondary group-hover:text-luxury-gold transition-colors">Account Details</span>
                    <div class="grid size-8 place-items-center rounded-xl bg-luxury-gold/10 text-luxury-gold">
                        <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <div class="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="font-display text-base font-bold text-white">Next Session</h2>
                                <p class="text-xs text-luxury-secondary/70">Your scheduled appointments</p>
                            </div>
                        </div>
                        <a href="{{ route('appointments.index') }}" class="text-xs font-display font-bold uppercase tracking-wider text-luxury-gold hover:text-white">View all</a>
                    </div>

                    @if($upcomingAppointments->isEmpty())
                        <div class="py-8 text-center flex flex-col items-center gap-3">
                            <span class="text-xs text-luxury-secondary/70 italic">No upcoming appointments scheduled.</span>
                                ...
                                <a href="{{ route('appointments.show', $appointment) }}" class="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-xs transition hover:border-luxury-gold/40 hover:bg-white/[0.04]">
                                    <div class="flex items-center gap-3">
                                        <div class="flex size-10 flex-col items-center justify-center rounded-lg bg-white/[0.06] text-luxury-gold font-display">
                                            <span class="text-[9px] font-bold uppercase">{{ $appointment->appointment_at->format('M') }}</span>
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            ...
                                            <span class="text-[11px] text-luxury-secondary">{{ $appointment->appointment_at->format('H:i') }} · Gentleman Master Barber</span>
                                        </div>
                                    </div>
                ...
                <div>
                    <div class="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="font-display text-base font-bold text-white">Recent Orders</h2>
                                <p class="text-xs text-luxury-secondary/70">Shop product purchases</p>
                            </div>
                        </div>
                        <a href="{{ route('orders.index') }}" class="text-xs font-display font-bold uppercase tracking-wider text-luxury-gold hover:text-white">View all</a>
                    </div>

                    @if($recentOrders->isEmpty())
                        <div class="py-8 text-center flex flex-col items-center gap-3">
                            <span class="text-xs text-luxury-secondary/70 italic">No shop orders placed yet.</span>
                            <a href="{{ route('products.index') }}" class="inline-flex items-center gap-1 text-xs font-bold text-luxury-gold hover:text-white">
                                Browse grooming products &rarr;
                            </a>
                        </div>
                    @else
                        <div class="space-y-3">
                            @foreach($recentOrders as $order)
                                <a href="{{ route('orders.show', $order) }}" class="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 text-xs transition hover:border-luxury-gold/40 hover:bg-white/[0.04]">
                                    <div class="flex flex-col gap-0.5">
                                        ...
                                        <span class="text-[11px] text-luxury-secondary">{{ $order->created_at->format('M d, Y') }}</span>
                                    </div>
                                    <div class="flex items-center gap-3">
